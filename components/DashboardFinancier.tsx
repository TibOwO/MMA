...
export default function DashboardFinancier() {
  ...
  // Page de détail de l'utilisateur selon le rôle :
  // admin -> gestion des utilisateurs, coach -> ses adhérents.
                    ...
                    <tbody>
                      {data.utilisateurs_echeances_en_attente.map((user, idx) => (
                        <tr
                          ...
                          className={`border-b border-gray-800 hover:bg-gray-800/50 ${user.user_id ? "cursor-pointer" : ""}`}
                          onClick={() => {
                            if (user.user_id) {
                              router.push(userDetailHref(user.user_id));
                            }
                          }}
                        >
                          <td className="py-3 px-4">
